...
const SingleListItem = ({ item }: { item: Product }) => {
  ...
  return (
    <div className="group rounded-xl border border-gray-200/80 bg-gray-50/30 shadow-sm transition-all duration-300 hover:shadow-md hover:border-[#FF0000]/25 overflow-hidden">
      <div className="flex flex-col sm:flex-row">
        <div className="relative overflow-hidden flex items-center justify-center w-full sm:max-w-[270px] sm:min-h-[270px] min-h-[220px] bg-white/60">
          <Link href={`/shop?productId=${item.id}`} className="flex items-center justify-center w-full h-full p-4">
            <Image
              src={item.img}
              alt={item.title}
              width={250}
        ...
        <div className="flex-1 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 py-5 px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col gap-1">
            <Link href={`/shop?productId=${item.id}`}>
              <h3 className="font-medium text-dark transition-colors duration-200 hover:text-[#FF0000] line-clamp-2">
                {item.title}
              </h3>
            </Link>
