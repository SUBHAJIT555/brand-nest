...
const SingleItem = ({ item }: { item: Product }) => {
  ...
  return (
    <div className="group">
      <div className="relative overflow-hidden rounded-2xl border-2 border-gray-200 bg-white min-h-[380px] shadow-md transition-all duration-300 hover:shadow-xl hover:border-[#FF0000]/30">
        {/* Bestseller ribbon */}
        <div className="absolute top-4 left-0 z-10 px-3 py-1 bg-[#FF0000] text-white text-xs font-bold uppercase tracking-wider rounded-r-md shadow">
          Bestseller
        </div>
        {/* Wishlist – top right */}
        <button
        ...
        <div className="flex justify-center items-center py-2">
          <Link href="/shop" className="block">
            <Image src={item.img} alt={item.title} width={280} height={280} className="object-contain transition-transform duration-300 group-hover:scale-105" />
          </Link>
        </div>

        <div className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-full flex flex-col gap-2 p-3 transition-transform duration-300 group-hover:translate-x-0">
          <button
            onClick={() => {
