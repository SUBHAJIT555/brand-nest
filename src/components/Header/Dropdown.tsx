...
const Dropdown = ({ menuItem, stickyMenu }) => {
  ...
  return (
    ...
    >
      <a
        href="#"
        className={`hover:text-[#FF0000] text-custom-sm font-medium text-dark flex items-center gap-1.5 capitalize transition-colors ${stickyMenu ? "xl:py-4" : "xl:py-5"
          } ${pathUrl.includes(menuItem.title) && "!text-[#FF0000]"}`}
      >
        {menuItem.title}
        <svg
      ...
      >
        {menuItem.submenu.map((item, i) => (
          <li key={i}>
            <Link
              href={item.path}
              className={`flex text-custom-sm hover:text-[#FF0000] hover:bg-[#FF0000]/5 py-[7px] px-4.5 rounded-md transition-colors ${pathUrl === item.path && "text-[#FF0000] bg-[#FF0000]/5"
                }`}
            >
              {item.title}
            </Link>
